...
SAAS_OFFERING_TO_USE="${SAAS_OFFERING_NAME}-global"
UNIT_LOCATION="global"
if [[ "${UNIT_KIND_TO_USE}" == *"-regional"* ]]; then
    SAAS_OFFERING_TO_USE="${SAAS_OFFERING_NAME}-regional"
...
if ! gcloud beta saas-runtime units describe "${SAAS_UNIT_NAME}" \
    ...
    echo "Creating Unit '${SAAS_UNIT_NAME}' to provision VM '${INSTANCE_NAME}' using Unit Kind '${UNIT_KIND_TO_USE}'..."
